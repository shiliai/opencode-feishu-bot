...
export function extractFeishuEventId(
  event: FeishuMessageReceiveEvent,
): string | null {
  return event.header?.event_id ?? getNestedString(event, ["event_id"]) ?? null;
}

export function extractCardActionDedupKey(
  event: Record<string, unknown>,
): string | null {
  const eventId =
    getNestedString(event, ["event_id"]) ??
    getNestedString(event, ["header", "event_id"]);
  if (eventId) {
    return eventId;
  }
  ...
  const tenantKey =
    getNestedString(event, ["tenant_key"]) ??
    getNestedString(event, ["context", "tenant_key"]);
  ...
  const actionValue =
    getNestedValue(event, ["action", "value"]) ??
    getNestedValue(event, ["action", "name"]);
  const formValue =
    getNestedValue(event, ["action", "form_value"]) ??
    getNestedValue(event, ["form_value"]);
...
export class FeishuEventRouter {
  ...
  handleMessageReceived(event: FeishuMessageReceiveEvent): void {
    ...
    if (eventId && !this.deduplicator.claim(`ws:${eventId}`)) {
      this.logger.debug(
        `[FeishuEventRouter] Dropped duplicate websocket event: ${eventId}`,
      );
      return;
    }

    this.scheduleAsync(() => {
      void Promise.resolve(this.onMessageReceived?.(event)).catch(
        (error: unknown) => {
          this.logger.error(
            "[FeishuEventRouter] Failed to process websocket event",
            error,
          );
        },
      );
  ...
  async handleCardAction(event: unknown): Promise<unknown> {
    ...
    try {
      return (
        (await this.onCardAction?.(record)) ?? this.emptyCardActionResponse
      );
    } catch (error) {
      this.logger.error(
        "[FeishuEventRouter] Failed to process card action",
        error,
      );
      return this.emptyCardActionResponse;
    }
