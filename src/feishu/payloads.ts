import { InteractiveCard, InteractiveCardMarkdownElement } from "@larksuiteoapi/node-sdk";
...
export function buildPostPayload(title: string, paragraphs: string[][], locale: string = "zh_cn"): string {
  const content = paragraphs.map(p => p.map(text => ({ tag: "text", text })));
...
export function buildCardPayload(card: InteractiveCard, updateMulti: boolean = false): string {
  if (updateMulti) {
    card.config = {
      ...(card.config || {}),
      update_multi: true,
...
export function splitTextPayload(text: string, maxSize = MAX_TEXT_PAYLOAD_SIZE - 100): string[] {
...
export function truncateCardPayload(card: InteractiveCard, updateMulti: boolean = false): string {
  ...
  if (clonedCard.elements && clonedCard.elements.length > 1) {
    while (clonedCard.elements.length > 1 && Buffer.byteLength(buildCardPayload(clonedCard, updateMulti), 'utf8') > MAX_CARD_PAYLOAD_SIZE) {
  ...
  if (Buffer.byteLength(finalPayload, 'utf8') > MAX_CARD_PAYLOAD_SIZE && clonedCard.elements && clonedCard.elements.length === 1) {
    ...
    if (el.tag === 'markdown' && typeof el.content === 'string') {
       ...
       const overage = Buffer.byteLength(finalPayload, 'utf8') - MAX_CARD_PAYLOAD_SIZE;
       const charsToRemove = overage + 100; // buffer
       el.content = el.content.slice(0, Math.max(0, el.content.length - charsToRemove)) + '... [Truncated]';
